...
class EmployeePortal(CustomerPortal):
    ...
    def portal_my_attendances(self, **kw):
        ...
        has_today_attendance = bool(today_attendances)
        has_open_attendance = any(not att.check_out for att in today_attendances)
        ...
        return request.render(
            "ibec_portal_empleado.portal_attendances_template",
            values
        )
    ...
    def portal_attendance_clock(self):
        ...
        if employee.attendance_state == 'checked_in':
            ...
            if not attendance:
                return {
                    'error': 'No se encontró un registro de entrada activo.',
                    'state': 'checked_out',
                }
        ...
        already_today = request.env['hr.attendance'].search_count([
            ('employee_id', '=', employee.id),
            ('check_in', '>=', day_start),
            ('check_in', '<=', day_end),
        ])
        ...
        if already_today:
            return {
                'error': 'Ya existe un registro de entrada para hoy. No puedes volver a registrar la entrada o salida .',
                'state': 'checked_in',
            }
